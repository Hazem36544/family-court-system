    ...
    return (
        <header className="bg-[#1e3a8a] rounded-[2rem] p-6 md:p-8 mb-8 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-6 md:gap-0">
            ...
            <div className="flex items-center gap-4 md:gap-6 z-10 w-full md:w-auto">
                <button onClick={onBack} className="bg-white/10 hover:bg-white/20 text-white p-3 rounded-xl transition-all shadow-inner border border-white/10 shrink-0 cursor-pointer outline-none group">
                    <ChevronRight size={24} className="md:w-7 md:h-7 group-hover:-translate-x-1 transition-transform" />
                </button>
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">حساب المحكمة</h1>
                    <p className="text-blue-200 text-xs md:text-sm font-medium flex items-center gap-1.5 md:gap-2">
                        <ShieldCheck size={16} className="shrink-0" />
                        إدارة معلومات الفرع وإعدادات الأمان
                    </p>
                </div>
            </div>

            <div className="bg-white/10 p-5 rounded-2xl border border-white/20 backdrop-blur-sm z-10 hidden md:block shadow-inner">
                <Scale size={40} strokeWidth={1.5} className="text-blue-100" />
            </div>
        </header>
    );
};
